...
### 3. TURN Shared Secret

The shared secret used for coturn `use-auth-secret` should be regenerated before production.

**Why:**
- TURN credentials are now short-lived, but they are derived from a single shared secret
- Production should start with a fresh secret that was never present in an old dev flow

**Required actions:**
- Generate a new random TURN shared secret
- Update the runtime Secrets Manager JSON secret
- Restart / redeploy signaling server and coturn so both use the same new secret
...
```json
{
  ...
  "TURN_SHARED_SECRET": "<new-random-turn-secret>",
...
5. Confirm coturn starts and can issue valid relay credentials
...
- No hardcoded JWT, DB, or TURN secrets remain in source or docs
...
- The signaling server, Lambda, and coturn all run successfully with the rotated secrets
